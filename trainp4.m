...
plot_data(X,Y);
##############################################
methods={"Initial hyp values","2nd reduction","3rd reduction","4th reduction","5th reduction"};
idalpha=0.0005;
ialpha=0.5;
ibeta2=0.99;
ibeta1=0.9;
imini=32;
hold off;
  alpha_values = []; # initialize the alpha values vector
...
################
    ...
    ## Randomly adjust hyperparameters by 5%

    alpha_values(end+1) = ialpha; # save alpha value
    idalpha = idalpha * ((0.1));
    ialpha = ialpha * ((0.1));
    ibeta2 = ibeta2 * ((0.1));
    ibeta1 = ibeta1 * ((0.1));
...
## display alpha values after all iterations
disp("Alpha values:")
disp(alpha_values)
xlabel("Iteration");
ylabel("Loss");
